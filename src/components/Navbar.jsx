            ...
            <nav className="flex justify-between items-center">
                ...
                <div className="relative flex items-center md:space-x-3 space-x-4">
                    <div >
                        {
                            user ? <>
                                <button onClick={() => { setIsDropDownOpen(!isDropDownOpen) }}><img src={avatar} alt="" className="size-6 rounded-full ring-2 ring-blue-500" /></button>
                                {
                                    isDropDownOpen &&
                                    (
                                            ...
                                            <ul className="py-2">
                                                {
                                                    dropDown.map((items) => {
                                                        return (
                                                            <li key={items.name} onClick={() => setIsDropDownOpen(!isDropDownOpen)}>
                                                                <Link to={items.href} className="block px-4 py-2 text-sm hover:bg-gray-200">{items.name}</Link>
                                                            </li>
                                                        )
                                                    })
                                                }
